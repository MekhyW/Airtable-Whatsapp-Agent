...
def run(
    ...
    uvicorn.run("airtable_whatsapp_agent.api.main:create_app", factory=True, host=host, port=port, reload=reload, workers=workers if not reload else 1, log_level=log_level,)
...
def health():
    ...
    health_checks = [
        ...
        ("WhatsApp Token", "✅ Configured" if settings.whatsapp.access_token else "❌ Missing"),
        ("Airtable API Key", "✅ Configured" if settings.airtable.api_key else "❌ Missing"),
    ]
